...
class SalesAnalysisDashboard extends Component {
  ...
  loadTransactions = async (cashier = null) => {
    try {
      const res = await fetch('http://localhost:4000/api/transactions');
      let transactions = await res.json();
      if (cashier) {
        transactions = transactions.filter(t => t.cashier_id === cashier._id || t.cashier_name === cashier.username);
      }
      this.computeMetrics(transactions);
    } catch (e) {
      this.computeMetrics([]);
    }
  }

  handleCashierChange = async (e) => {
    const { cashiers } = this.state;
    const selected = cashiers.find(c => c._id === e.target.value);
    await this.setState({ cashier: selected });
    await this.loadTransactions(selected);
  };
  ...
  computeMetrics = (transactions = []) => {
    ...
    // ...le reste du code reste inchangé, remplacer sampleData par transactions dans tous les calculs...
  ...
  render() {
    const { period, dailyCharts, dailyMetrics, weeklyMetrics, chartOptions, cashiers, cashier } = this.state;
    const days = ['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'];

    // Sélecteur de caissier
    const cashierSelector = (
      <div className="row">
          ...
          <div className="card">
            <div className="card-body">
              <div className="mb-3">
                ...
                <select className="form-control" value={cashier ? cashier._id : ''} onChange={this.handleCashierChange}>

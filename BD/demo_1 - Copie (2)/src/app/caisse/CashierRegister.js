...
class CashierRegisterSingleStore extends Component {
  ...
  handleCashierChange = async (e) => {
    ...
    // Charger les transactions du caissier sélectionné
    if (selected) {
      try {
        const res = await fetch('http://localhost:4000/api/transactions');
        const allTx = await res.json();
        const cashierTransactions = allTx.filter(t => t.cashier_id === selected._id || t.cashier_name === selected.username);
        this.setState({ transactions: cashierTransactions });
      } catch (e) {
  ...
  render() {
    const { cashier, cashiers, cart, currentProduct, qrInput, transactions, totalAmount, discount, paymentMethod, lastTransaction, showReceipt } = this.state;
    const subtotal = cart.reduce((sum, item) => sum + item.total, 0);

...
class CashierRegister extends Component {
  ...
  async componentDidMount() {
    // Charger la liste des caissiers dynamiquement
    try {
  ...
  completeTransaction = async () => {
    ...
    try {
      ...
      this.setState(prevState => ({
        transactions: [saved, ...prevState.transactions],
        lastTransaction: saved,
        showReceipt: true,
        cart: [],
        totalAmount: 0,
        discount: 0,
  ...
  render() {
    ...
    const { cashier, cashiers, cart, currentProduct, totalAmount, discount, paymentMethod, showReceipt, lastTransaction, transactions } = this.state;
                ...
                <div className="row">
                  <div className="col-md-6">
                    <div className="mb-3">
                      ...
                      <select className="form-control" value={cashier ? cashier._id : ''} onChange={e => {
                        const selected = cashiers.find(c => c._id === e.target.value);
                        this.setState({ cashier: selected });
                      }}>
